...
class Pivots(object):
    ...
    def daily_pivot_point(self):
        dailyHigh = self.df.iloc[-1]['High']
        dailyLow = self.df.iloc[-1]['Low']
        dailyClose = self.df.iloc[-1]['Close']
        dailyPP = (dailyHigh+dailyLow+dailyClose)/3
        return dailyPP
    
    def RL_1D(self):
        RL_1D = (2*self.dailyPP) - self.df.iloc[-1]['Low']
        return RL_1D
    
    def SL_1D(self):
        SL_1D = (2*self.dailyPP) - self.df.iloc[-1]['High']
        return SL_1D
    ...
    def dailyPivotsDF(self):
        ...
        dailyPPRS.to_csv(os.path.join(settings.CSV_DIR,"%s_%s_%s.csv" % (
                self.pair,'DailyPivots',datetime.now().date())),index=False)
        print dailyPPRS 
        return dailyPPRS
    ...
    def weeklyPivotsDF(self):
        ...
        weeklyPPRS.to_csv(os.path.join(settings.CSV_DIR,"%s_%s_%s.csv" % (
                self.pair,'WeeklyPivots',datetime.now().date())),index=False)
        print weeklyPPRS
        return weeklyPPRS
